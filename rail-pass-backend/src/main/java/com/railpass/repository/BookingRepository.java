...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT COUNT(p) FROM Booking b JOIN b.passengers p WHERE b.train = :train AND b.journeyDate = :journeyDate AND b.classType = :classType AND b.status = 'CONFIRMED' AND p.status = 'CONFIRMED'")
